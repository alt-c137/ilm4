...
class SiteSettings(SingletonModel):
    ...
    phone_for_publish = models.BooleanField(
        'подтверждённый номер — для публикаций', default=True,
        help_text='Подавать объявления, вакансии, услуги и т.д. можно только с номером, подтверждённым '
                  'через Telegram (один номер — один аккаунт). Смотреть можно всем. Работает, когда '
                  'подключён Telegram-бот; сотрудников не касается')
    phone_for_nikah = models.BooleanField(
