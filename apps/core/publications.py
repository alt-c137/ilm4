...
PUBLICATIONS = [
    Pub('buy', _lazy('Объявления'), 'market.Listing', url='market:detail', form='apps.market.forms.ListingForm',
        active='is_active', create='/buy/add/'),
    Pub('jobs', _lazy('Вакансии'), 'jobs.Vacancy', url='jobs:detail', form='apps.jobs.forms.VacancyForm', create='/jobs/add/'),
    Pub('services', _lazy('Услуги и фриланс'), 'services.Service', title='name', url='/services/',
    ...
    Pub('doctors', _lazy('Врачи'), 'health.Doctor', title='name', url='health:detail', form='apps.health.forms.DoctorForm',
        create='/health/add/'),
    Pub('stories', _lazy('Истории переезда'), 'migration.Story', owner='author', url='migration:detail',
        form='apps.migration.forms.StoryForm', create='/migration/add/'),
    Pub('books', _lazy('Книги'), 'library.Book', url='/library/', create='/library/add/'),
    Pub('topics', _lazy('Вопросы на форуме'), 'forum.Topic', owner='author', url='forum:detail',
