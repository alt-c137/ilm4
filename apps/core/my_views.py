...
STATUS_LABEL = {Moderation.PENDING: (_lazy('На проверке'), 'pending'), Moderation.APPROVED: (_lazy('Опубликовано'), 'ok'),
                Moderation.REJECTED: (_lazy('Отклонено'), 'bad')}
    ...
    for pub in PUBLICATIONS:
        ...
        for obj in model.objects.filter(**{pub.owner: request.user}).order_by('-pk')[:100]:
            ...
            items.append({'obj': obj, 'title': pub.title_of(obj), 'url': pub.url_of(obj),
                          'status': _('Скрыто вами') if hidden else label, 'css': 'off' if hidden else css,
...
def my_edit(request, key, pk):
    ...
    if form_cls is None:
        messages.info(request, _('Эту публикацию можно только удалить и добавить заново.'))
...
def my_delete(request, key, pk):
    ...
    messages.success(request, _('«{title}» удалено.').format(title=title))
    return redirect('core:my')
...
def my_toggle(request, key, pk):
    ...
    messages.success(request, _('Снова показывается.') if getattr(obj, pub.active) else _('Скрыто — никто не видит.'))
    return redirect('core:my')
...
def report(request):
    ...
    if reason not in dict(Report.REASONS):
        messages.error(request, _('Выберите причину жалобы.'))
        return redirect(back)
    key = f'reports:{request.user.pk}'
    if cache.get(key, 0) >= REPORTS_PER_DAY:
        messages.error(request, _('Слишком много жалоб за сегодня. Мы уже разбираемся.'))
        return redirect(back)
...
def _after_report(ct, obj, reason):
    ...
    try:
        ...
        if chat:
            ...
            api('sendMessage', {'chat_id': chat, 'text': _('🚩 Жалоба ({v1}): {v3} #{v5} — {v7}. Всего жалоб: {count}. Разобрать: админка → Жалобы.').format(v1=dict(Report.REASONS)[reason], v3=ct.model, v5=obj.pk, v7=str(obj)[:120], count=count)})
    except ImportError:
        pass
